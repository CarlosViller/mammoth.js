...
function read(docxFile, input, options) {
    ...
    return promises.props({
        ...
        return {
            ...
            headers: readXmlFilesWithBody(result.partPaths.headers, result, function(bodyReader, xml) {
                if (xml) {
            ...
            footers: readXmlFilesWithBody(result.partPaths.footers, result, function(bodyReader, xml) {
                if (xml) {
...
function readXmlFilesWithBody(filenames, options, func) {
    var filenamesArray = Array.isArray(filenames) ? filenames : [filenames];
    
    if (filenamesArray.length === 0) {
        return promises.resolve(new Result([]));
    }
    
    return promises.mapSeries(filenamesArray, function(filename) {
